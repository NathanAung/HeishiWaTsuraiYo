...
void Main()
{
	...
	const Texture knightAtkTexture{ U"Assets/knightAtk.png"};
	...
	while (System::Update())
	{
		...
		if (KeySpace.down()) {
			atkAnimInterval = 8;
			knightAtkTexture.scaled(0.2).mirrored(facing).drawAt(playerPos);
		}
		else if (atkAnimInterval > 0){
			atkAnimInterval-= Scene::DeltaTime() * 0.01;
			knightAtkTexture.scaled(0.2).mirrored(facing).drawAt(playerPos);
		}
		else {
